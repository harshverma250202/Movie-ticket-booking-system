...
class AddShow(APIView):
    ...
    def post(self, request):
        try:
            copy = request.data
            copy['salesperson'] = request.data['salesperson']
...
class ExpenditureView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if(not show):
                return Response({"message": "show not found"}, status=status.HTTP_400_BAD_REQUEST)
            exp = Expenditure(request.data, show=show)
            exp.save()
            return Response({"message": "expenditure added successfully"}, status=status.HTTP_200_OK)
